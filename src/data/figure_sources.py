...
def format_figure_value(name: str, value: float) -> str:
    ...
    verified-figures document) must agree on this, so unit awareness is defined here ONCE. Rupee
    figures use the shared Indian crore/lakh formatter (see src/formatting.py)."""
    from ..formatting import format_rupees_crore_lakh
    if name in RATIO_FIGURES:
        return f"{value:.1f}x"
    if name in PERCENT_FIGURES:
        return f"{value:.1f}%"
    return format_rupees_crore_lakh(value)
